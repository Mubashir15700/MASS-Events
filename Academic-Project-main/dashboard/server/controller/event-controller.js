...
export const getEventBooking = async (req, res) => {
    try {
        ...
        const bookedStaffs = await Staff.find({ _id: { $in: bookedStaffsIds } }).select('-password').sort({ 
            category: -1 
        });
...
export const getEventAttendance = async (req, res) => {
    try {
        ...
        const attendedStaffs = await Staff.find({ _id: { $in: attendedStaffsIds } }).select('-password').sort({ 
            category: -1 
        });
